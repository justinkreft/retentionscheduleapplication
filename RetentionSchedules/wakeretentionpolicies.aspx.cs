...
{
    public partial class wakeretentionpolicies : System.Web.UI.Page
    {
        ...
        protected void Page_LoadComplete(object sender, EventArgs e)
        {
            ...
            policygroupUIDclause = "PolicyGroupUID " + ddlPolicyGroup.SelectedValue.ToString();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                Title.Text = ((HiddenField)previousPage.FindControl("TitleString")).Value.ToString();
            ...
            else
            {
                if (!IsPostBack)
                {
                    Title.Text = "No Search Scope Defined: Showing All Records";
                }
            }
            //Construct Drop down lists
            if (!IsPostBack)
            {
                ddlQuery.SelectCommand = "SELECT DISTINCT PolicyGroupUID, PolicyGroupName FROM divisionlkup WHERE PolicyGroupUID " + valuestring + " ORDER BY PolicyGroupName";
                //Response.Write("ddlPolicyGroup ddlQuery.SelectCommand (PageLoad): " + ddlQuery.SelectCommand.ToString() + "<br>");
                ...
                if (sqlReader.HasRows)
                {
                    while (sqlReader.Read())
                    {
                        ...
                        ddlPolicyGroup.Items.Add(item);
                    }
                    if (ddlPolicyGroup.Items.Count > 1)
                    {
                        ListItem policygroupall = new ListItem("Narrow results by schedule...", " != ''", true);
                        policygroupall.Selected = true;
                        ddlPolicyGroup.Items.Add(policygroupall);
                    }
                }
            ...
            if (ddlPolicyGroup.SelectedValue.ToString() != " != ''")
            {
                SqlDataScope.SelectCommand = "SELECT DISTINCT CONCAT(DepartmentName, ': ', DivisionName) FROM divisionlkup WHERE PolicyGroupUID " + ddlPolicyGroup.SelectedValue.ToString();
                SqlDataReader sqlReader = (SqlDataReader)SqlDataScope.Select(DataSourceSelectArguments.Empty);
                if (sqlReader.HasRows)
                {
        ...
        protected void DetailsView1_DataBound(object sender, EventArgs e)
        {
            ...
            {
                if (DetailsView1.Rows[7].Cells[1].Text == DateTime.Today.ToString("yyyy-MM-dd"))
                {
                    DetailsView1.Rows[7].Cells[1].Text = "Current";
                }
                if (DetailsView1.Rows[16].Cells[1].Text == DateTime.Today.ToString("yyyy-MM-dd"))
                {
                    DetailsView1.Rows[16].Cells[1].Text = "Current";
                }
                if (DetailsView1.Rows[27].Cells[1].Text == DateTime.Today.ToString("yyyy-MM-dd"))
                {
                    DetailsView1.Rows[27].Cells[1].Text = "Current";
                }
                if (DetailsView1.Rows[36].Cells[1].Text == DateTime.Today.ToString("yyyy-MM-dd"))
                {
                    DetailsView1.Rows[36].Cells[1].Text = "Current";
                }
            }
        }
    }
